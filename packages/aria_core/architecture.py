...
_ARCHITECTURE_LAYERS: Tuple[ArchitectureLayer, ...] = (
    ...
    ArchitectureLayer(
        ...
        current_state="Rich/prompt_toolkit are optional, but output truncation, repeated summaries, and file-opening UX need hardening.",
        status=LayerStatus.PARTIAL,
        source_paths=("ui/", "apps/cli/commands/"),
        next_steps=("Move terminal panels and generated-file open actions behind a UI service.",),
    ),
    ArchitectureLayer(
        name="context",
        ...
        current_state="The CLI warns near token limits but does not yet run automatic compaction and resume contracts.",
        status=LayerStatus.PLANNED,
        source_paths=("context_store.py", "memory.py", "conversation_store.py"),
        next_steps=("Add ContextService with compaction thresholds, summary checkpoints, and artifact references.",),
        blockers=("Define compact/resume schema that local and cloud models both follow.",),
    ),
    ArchitectureLayer(
        name="runtime",
        ...
        current_state="The agent loop still lives mostly in aria_cli.py with mixin adapters around it.",
        status=LayerStatus.PARTIAL,
        source_paths=("aria_cli.py", "runtime/"),
        depends_on=("settings", "tools", "safety", "context"),
        next_steps=("Extract AgentRuntime behind packages.aria_services.registry runtime boundary.",),
    ),
    ArchitectureLayer(
        name="tools",
