...
_ARCHITECTURE_LAYERS: Tuple[ArchitectureLayer, ...] = (
    ...
    ArchitectureLayer(
        ...
        current_state="A public packages.aria_sdk facade now owns SDK-style query/result events; the full tool loop still lives mostly in aria_cli.py with mixin adapters around it.",
        status=LayerStatus.PARTIAL,
        source_paths=("aria_cli.py", "runtime/", "packages/aria_sdk/", "apps/cli/deterministic.py"),
        depends_on=("settings", "tools", "safety", "context"),
        next_steps=("Move the remaining model/tool loop from aria_cli.py behind AriaSDKClient and typed runtime services.",),
    ),
    ArchitectureLayer(
        name="tools",
