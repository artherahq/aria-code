...
修好 TLS 之后重新测量,分数从 **6/7 (86%)** 变成 **6/10 (60%)**。

这不是退步,是之前那个 86% 在撒谎:通过数一样是 6,区别在于原先有 3 次尝试因 SSL 故障记成
ERROR 被排除在分母外。**修掉噪声之后分数下降,是分数开始变诚实的样子。**

这也正是四态设计要的效果 —— 如果当初把 ERROR 算成 FAIL,那 3 次会被记在模型头上;
算成 PASS 则会掩盖它们。两种做法都会让「修好网络」这件事看起来毫无变化。

### 被截断的运行不是一次失败

紧接着又发现同一个任务单独跑 3/3、在套件里 1/2。时间数据解释了它:单跑 78–146 秒,
套件里连续请求互相争抢并被限流,超过了 600 秒预算。

我最初把 solver 超时判成 FAIL,理由写在注释里:「它有过时间,什么都没产出」。
**这个理由只在预算相对于工作量足够宽裕时成立**,而这里并不成立 ——
那样等于因为 harness 把模型掐断而怪模型,恰恰是 PASS/FAIL 与 ERROR 之分要防的那种混淆。
超时现在报 ERROR。

（但如果工作在超时之前就已经落盘且检查是绿的,依然算 PASS ——
检查始终是唯一的事实来源。）

## 改测试不算解题

在 Gemini 上跑的时候,`stripe-refund-validation` 这一轮 agent **改的是 `test_settlement.py`,
不是被测模块**。改动追踪一眼就看出来了——而在那之前,harness 会给它记一个 PASS。

这是记分牌变成谎话最经典的一条路:检查就是那些测试,能改测试就能把任何任务弄绿。
所以任务现在有 `protect`,默认保护 `test_*.py` / `*_test.py` / `tests/**`,
**动了受保护文件一律判 FAIL,无论检查是不是绿的**。需要真的改测试的任务可以显式
`protect: []` 关掉,但那必须是一个人做的决定,而不是默认。

### 改输入数据也不算解题

`reconcile-waybills` 有一次「通过」的方式是同时改了 `test_reconcile.py`、`waybills.csv`
和 `invoice.csv` —— **把发票改到和账对上**。这不是修好了对账,是把账做平了,
而这恰恰是这个任务要检测的那种失败。

所以 `protect` 不只是保护测试:**凡是构成「题目」的文件都要保护**。
对一个对账任务来说,输入数据就是题目。
