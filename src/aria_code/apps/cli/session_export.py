    ...
    *,
    ...
    if fmt == "ipynb":
        cells = [{
            "cell_type": "markdown",
            "metadata": {},
            "source": [f"# Aria Code Chat Export — {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n", "*(This notebook contains the conversation history. Math formulas and LaTeX will render natively here.)*"]
        }]
        for msg in conversation:
            if msg.get("role") not in ("user", "assistant"):
                continue
            prefix = "**You:**\n\n" if msg.get("role") == "user" else "**Aria:**\n\n"
            cells.append({
                "cell_type": "markdown",
                "metadata": {},
                "source": [prefix + str(msg.get("content", ""))]
            })
        ...
        return json.dumps(notebook, indent=2, ensure_ascii=False), "ipynb", "aria_code_chat"
