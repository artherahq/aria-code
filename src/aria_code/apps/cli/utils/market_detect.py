...
_COMPANY_TO_TICKER = {
    ...
    "携程港股": "9961.HK",
    # "汇丰" 这个裸名此前在这里和美股表（HSBC）各出现一次，后者在文件靠后、
    # 静默覆盖了这里，导致"汇丰"解析成美股 ADR 而"汇丰控股"解析成港股——
    # 同一家公司两个结果。港股 0005.HK 是主上市地，裸名归它；美股表那边
    # 保留 "汇丰银行"/"汇丰ADR" 等限定说法。
    ...
    "Workday": "WDAY",
    "Snowflake": "SNOW",
    # Databricks 在下方 PRIVATE 表里已有规范条目，此处的 "未上市"
    # 被静默覆盖；删掉以免两处维护。
    ...
    "诺基亚": "NOK",
    "爱立信": "ERIC",
    "汇丰银行": "HSBC", "汇丰ADR": "HSBC",   # 裸名"汇丰"归港股主上市地，见上文
    "巴克莱": "BCS",
    "德意志银行": "DB", "德银": "DB",
